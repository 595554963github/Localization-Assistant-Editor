...
class BinaryEditorApp:
    ...
    def create_widgets(self):
        ...
        edit_menu.add_command(label="查找上一个", command=self.find_prev, accelerator="F2")
        edit_menu.add_command(label="查找下一个", command=self.find_next, accelerator="F3") 
        edit_menu.add_command(label="替换当前", command=self.replace_current, accelerator="F4")
        edit_menu.add_command(label="替换全部", command=self.replace_all, accelerator="F5")
        edit_menu.add_separator()
        edit_menu.add_command(label="导出所有ASCII", command=self.export_ascii_text, accelerator="F6")
        edit_menu.add_command(label="导出指定范围ASCII", command=self.export_selected_range, accelerator="F7")
        edit_menu.add_command(label="导入ASCII", command=self.import_replace_file, accelerator="F8")
        edit_menu.add_separator()
        edit_menu.add_command(label="实时编码转换器", command=self.show_encoding_converter, accelerator="F9")
        edit_menu.add_separator()
        edit_menu.add_command(label="重置", command=self.reset_all, accelerator="F1")
        ...
        self.root.bind("<F2>", lambda e: self.find_prev())
        self.root.bind("<F3>", lambda e: self.find_next())
        self.root.bind("<F4>", lambda e: self.replace_current())
        self.root.bind("<F5>", lambda e: self.replace_all())
        self.root.bind("<F6>", lambda e: self.export_ascii_text())
        self.root.bind("<F7>", lambda e: self.export_selected_range())
        self.root.bind("<F8>", lambda e: self.import_replace_file())
        self.root.bind("<F9>", lambda e: self.show_encoding_converter())
        self.root.bind("<F1>", lambda e: self.reset_all())
        ...
        ttk.Label(search_frame, text="编码:").grid(row=1, column=2, sticky=tk.W, padx=(10, 20), pady=10)
        encoding_menu = ttk.Combobox(search_frame, textvariable=self.encoding_var,
                                     values=self.encoding_options, state="readonly", width=10)
        encoding_menu.grid(row=1, column=3, sticky=tk.W, padx=(0, 10), pady=5)
        encoding_menu.bind("<<ComboboxSelected>>", self.update_display)
        ...
        self.replace_button = ttk.Button(button_frame, text="替换当前", command=self.replace_current)
        self.replace_button.grid(row=0, column=3, padx=5, pady=5)

        self.replace_all_button = ttk.Button(button_frame, text="替换全部", command=self.replace_all)
        self.replace_all_button.grid(row=0, column=4, padx=5, pady=5)

        reset_button = ttk.Button(button_frame, text="重置", command=self.reset_all)
...
0xB1-±(正负)      0xB5-µ(微)      0xD7-×(乘)     0xF7-÷(除)"""
    ...
    def reset_all(self):
        ...
        self.replace_hex_entry.delete(0, tk.END)
        self.clear_matches()
        self.update_status("已重置所有查找和替换内容")
